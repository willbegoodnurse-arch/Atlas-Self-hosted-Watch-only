...
export function addBbqrFrame(state: BbqrCollectorState, frame: BbqrFrame): BbqrCollectorResult {
  if (state.total !== null && state.total !== frame.total) {
    return {
      state,
      status: "error",
      message: "BBQr frame total mismatch. Clear BBQr frames and scan one export again."
    };
  }
  if (state.encoding !== null && state.encoding !== frame.encoding) {
    return {
      state,
      status: "error",
      message: "BBQr encoding mismatch. Clear BBQr frames and scan one export again."
    };
  }
  if (state.fileType !== null && state.fileType !== frame.fileType) {
    return {
      state,
      status: "error",
      message: "BBQr file type mismatch. Clear BBQr frames and scan one export again."
    };
  }
